...
//  Orrery
//
//  Created by Jesse Wolfe on 3/9/14.
...
@implementation POEarth
- (NSArray*) coordinatesAtTime: (double)elapsed_time
{
    // This is cut&paste from POMoon.m
    // with some signs reversed
    // It could probably be DRYed up somehow.
    ...
    double x = -earth_barycenter_distance_au * cos(ecliptic_latitude) * cos(ecliptic_longitude);
    double y = -earth_barycenter_distance_au * cos(ecliptic_latitude) * sin(ecliptic_longitude);
    double z = -earth_barycenter_distance_au * sin(ecliptic_latitude);
